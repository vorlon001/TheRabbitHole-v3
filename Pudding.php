...
class Pudding{
	...
	return sprintf("[%s]/[%s-%s]/[%s]",($ip=$this->get_server('REMOTE_ADDR')),(geoip_country_code_by_name($ip)??'Not yet set'),(geoip_country_name_by_name($ip)??'Not yet set'),(geoip_asnum_by_name($ip) ?? 'Not yet set'));
    }
	...
	if(isset($this->_GET[$id])) return auth_replace(parser($this->_GET[$id],254));
	return NULL;
    }
    public function _post($id) {
	if(isset($this->_POST[$id])) return auth_replace(parser($this->_POST[$id],254));
	return NULL;
    }
    public function _cookie($id) {
        if(isset($this->_COOKIE[$id])) return auth_replace(parser($this->_COOKIE[$id],254));
	return NULL;
    }
    public function set_cookie($cookie,$data,$ttl) {
	setcookie($cookie, $data, time()+$ttl,'/');
    }
    public function _server($id) {
	if(isset($this->_SERVER[$id])) return $this->auth_replace($this->parser($this->_SERVER[$id],254));
	return NULL;
    }
    /**
    ...
    public function _ua() {
	$UA=parse_user_agent(substr($this->_SERVER['HTTP_USER_AGENT'],0,255));
